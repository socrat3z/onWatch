...
// migrateForkOverlaySchema executes schema extensions introduced by the fork-overlay
// without modifying upstream migrateSchema versions or sequence.
func (s *Store) migrateForkOverlaySchema() error {
	// 1. Anthropic and Antigravity now share the same account-scoped persistence
	...
	// 2. Store account-wide OpenRouter credits separately from per-key limits.
