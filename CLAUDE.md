...
- `internal/agent/anthropic_agent.go`: a resident Claude Code blocks a *proactive* refresh (its token is still live) but NOT recovery from repeated 401s (the stored token is already dead, so deferring only guarantees a permanently paused account). Recovery prefers a pair Claude Code has already written, and exchanges under the lock only when the store is still the rejected one.
